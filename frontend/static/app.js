...
function setupEventListeners() {
    ...
    // IP Threat Search
    document.getElementById('threatSearchInput').addEventListener('input', (e) => {
        state.threatFilterText = e.target.value.toLowerCase();
        renderThreatMatrix();
    });

    // Heatmap Reset
    document.getElementById('btnResetHeatmapFilter').addEventListener('click', () => {
...
function setupTunerControls() {
    const burstCount = document.getElementById('tuneBurstCount');
    const burstWindow = document.getElementById('tuneBurstWindow');
    const offStart = document.getElementById('tuneOffStart');
    const offEnd = document.getElementById('tuneOffEnd');
    const sensitivePaths = document.getElementById('tuneSensitivePaths');

    burstCount.addEventListener('input', (e) => {
        document.getElementById('valBurstCount').innerText = `${e.target.value} reqs`;
    });

    burstWindow.addEventListener('input', (e) => {
        document.getElementById('valBurstWindow').innerText = `${e.target.value}s`;
    });

    offStart.addEventListener('input', (e) => {
        const val = parseInt(e.target.value);
        const ampm = val >= 12 ? 'PM' : 'AM';
        const displayH = val % 12 === 0 ? 12 : val % 12;
        document.getElementById('valOffStart').innerText = `${String(val).padStart(2, '0')}:00 (${displayH} ${ampm})`;
    });

    offEnd.addEventListener('input', (e) => {
        const val = parseInt(e.target.value);
        const ampm = val >= 12 ? 'PM' : 'AM';
        const displayH = val % 12 === 0 ? 12 : val % 12;
        document.getElementById('valOffEnd').innerText = `${String(val).padStart(2, '0')}:00 (${displayH} ${ampm})`;
    });

    document.getElementById('btnApplyTuning').addEventListener('click', async () => {
        const payload = {
            burst_threshold_count: parseInt(burstCount.value),
            burst_window_seconds: parseInt(burstWindow.value),
            off_hours_start_hour: parseInt(offStart.value),
            off_hours_end_hour: parseInt(offEnd.value),
            sensitive_paths: sensitivePaths.value.split(',').map(s => s.trim()).filter(Boolean),
            rare_percentage_ratio: 0.015,
            rare_max_occurrences: 2
        };

        showToast('Applying live parameter thresholds & re-scanning...', 'info');
        try {
            ...
            if (res.status === 'success') {
                showToast(`Thresholds applied! ${res.flagged_count} anomalies detected with new parameters.`, 'success');
                await fetchDashboardData();
                switchView('logs');
            } else {
                showToast(res.error || 'Failed to tune thresholds', 'error');
            }
        } catch (err) {
...
async function fetchDashboardData() {
    try {
        const [summaryRes, logsRes, flaggedRes, threatsRes] = await Promise.all([
            ...
            fetch('/api/threats/ips').then(r => r.json())
        ]);
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="9" class="empty-state">
                    ...
                    <p>Try adjusting your search query, filters, or resetting the heatmap brush.</p>
                </td>
            </tr>
        `;
        lucide.createIcons();
